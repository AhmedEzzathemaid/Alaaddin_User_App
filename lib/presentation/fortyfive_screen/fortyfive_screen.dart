...
import '../fortyfive_screen/widgets/s18_item_widget.dart';
import 'bloc/fortyfive_bloc.dart';
import 'models/eightyseven_item_model.dart';
import 'models/fortyfive_model.dart';
...
import 'widgets/productlist9_item_widget.dart';
import 'widgets/rectangle5_item_widget.dart';

class FortyfiveScreen extends StatelessWidget {
  const FortyfiveScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<FortyfiveBloc>(
      create: (context) => FortyfiveBloc(FortyfiveState(
        fortyfiveModelObj: FortyfiveModel(),
      ))
        ..add(FortyfiveInitialEvent()),
      child: FortyfiveScreen(),
    );
  }
  ...
  Widget _buildClose(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          BlocBuilder<FortyfiveBloc, FortyfiveState>(
            builder: (context, state) {
              return CarouselSlider.builder(
                ...
                itemCount:
                    state.fortyfiveModelObj?.eightysevenItemList.length ?? 0,
                itemBuilder: (context, index, realIndex) {
                  EightysevenItemModel model =
                      state.fortyfiveModelObj?.eightysevenItemList[index] ??
                          EightysevenItemModel();
          ...
          Align(
            alignment: Alignment.bottomCenter,
            child: BlocBuilder<FortyfiveBloc, FortyfiveState>(
              builder: (context, state) {
                return Container(
                  height: 10.v,
                  ...
                  child: AnimatedSmoothIndicator(
                    ...
                    count:
                        state.fortyfiveModelObj?.eightysevenItemList.length ??
                            0,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Spacer(),
          BlocSelector<FortyfiveBloc, FortyfiveState, bool?>(
  ...
  Widget _buildS(BuildContext context) {
    return Align(
      ...
      child: Padding(
        padding: EdgeInsets.only(left: 20.h),
        child: BlocSelector<FortyfiveBloc, FortyfiveState, FortyfiveModel?>(
          selector: (state) => state.fortyfiveModelObj,
          builder: (context, fortyfiveModelObj) {
            return Wrap(
              runSpacing: 7.v,
              spacing: 7.h,
              children: List<Widget>.generate(
                fortyfiveModelObj?.s18ItemList.length ?? 0,
                (index) {
                  S18ItemModel model =
                      fortyfiveModelObj?.s18ItemList[index] ?? S18ItemModel();

                  return S18ItemWidget(
                    model,
                    onSelectedChipView: (value) {
                      context.read<FortyfiveBloc>().add(
  ...
  Widget _buildCharacteristics(BuildContext context) {
    return Padding(
      ...
      child:
          BlocSelector<FortyfiveBloc, FortyfiveState, TextEditingController?>(
        ...
        child: BlocSelector<FortyfiveBloc, FortyfiveState, FortyfiveModel?>(
          ...
          builder: (context, fortyfiveModelObj) {
            return ListView.separated(
              ...
              itemCount: fortyfiveModelObj?.rectangle5ItemList.length ?? 0,
              itemBuilder: (context, index) {
                Rectangle5ItemModel model =
                    fortyfiveModelObj?.rectangle5ItemList[index] ??
                        Rectangle5ItemModel();
                return Rectangle5ItemWidget(
                  model,
                );
              },
            );
        ...
        child: BlocSelector<FortyfiveBloc, FortyfiveState, FortyfiveModel?>(
          ...
          builder: (context, fortyfiveModelObj) {
            return ListView.separated(
              ...
              itemCount: fortyfiveModelObj?.productlist9ItemList.length ?? 0,
              itemBuilder: (context, index) {
                Productlist9ItemModel model =
                    fortyfiveModelObj?.productlist9ItemList[index] ??
                        Productlist9ItemModel();
                return Productlist9ItemWidget(
                  model,
                );
              },
            );
