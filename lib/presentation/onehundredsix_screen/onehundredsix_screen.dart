...
import 'bloc/onehundredsix_bloc.dart';
import 'models/onehundredsix_model.dart';
import 'models/productcard9_item_model.dart';
import 'widgets/productcard9_item_widget.dart';

class OnehundredsixScreen extends StatelessWidget {
  const OnehundredsixScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<OnehundredsixBloc>(
      create: (context) => OnehundredsixBloc(OnehundredsixState(
        onehundredsixModelObj: OnehundredsixModel(),
      ))
        ..add(OnehundredsixInitialEvent()),
      child: OnehundredsixScreen(),
    );
  }
  ...
  Widget _buildProductCard(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(left: 2.h),
      child: BlocSelector<OnehundredsixBloc, OnehundredsixState,
          OnehundredsixModel?>(
        selector: (state) => state.onehundredsixModelObj,
        builder: (context, onehundredsixModelObj) {
          return ListView.separated(
            ...
            itemCount: onehundredsixModelObj?.productcard9ItemList.length ?? 0,
            itemBuilder: (context, index) {
              Productcard9ItemModel model =
                  onehundredsixModelObj?.productcard9ItemList[index] ??
