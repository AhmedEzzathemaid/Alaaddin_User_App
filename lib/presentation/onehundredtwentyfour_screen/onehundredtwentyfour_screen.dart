...
import 'bloc/onehundredtwentyfour_bloc.dart';
import 'models/onehundredtwentyfour_model.dart';
import 'models/productdetails4_item_model.dart';
import 'widgets/productdetails4_item_widget.dart';

class OnehundredtwentyfourScreen extends StatelessWidget {
  const OnehundredtwentyfourScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<OnehundredtwentyfourBloc>(
      create: (context) => OnehundredtwentyfourBloc(OnehundredtwentyfourState(
        onehundredtwentyfourModelObj: OnehundredtwentyfourModel(),
      ))
        ..add(OnehundredtwentyfourInitialEvent()),
      child: OnehundredtwentyfourScreen(),
    );
  }
  ...
  Widget _buildProductDetails(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(left: 2.h),
      child: BlocSelector<OnehundredtwentyfourBloc, OnehundredtwentyfourState,
          OnehundredtwentyfourModel?>(
        selector: (state) => state.onehundredtwentyfourModelObj,
        builder: (context, onehundredtwentyfourModelObj) {
          return ListView.separated(
            ...
            itemCount:
                onehundredtwentyfourModelObj?.productdetails4ItemList.length ??
                    0,
            itemBuilder: (context, index) {
              Productdetails4ItemModel model = onehundredtwentyfourModelObj
                      ?.productdetails4ItemList[index] ??
                  Productdetails4ItemModel();
              return Productdetails4ItemWidget(
                model,
              );
            },
          );
