...
import 'bloc/onehundredtwo_bloc.dart';
import 'models/onehundredtwo_model.dart';
import 'models/productcard6_item_model.dart';
import 'widgets/productcard6_item_widget.dart';

class OnehundredtwoScreen extends StatelessWidget {
  const OnehundredtwoScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<OnehundredtwoBloc>(
      create: (context) => OnehundredtwoBloc(OnehundredtwoState(
        onehundredtwoModelObj: OnehundredtwoModel(),
      ))
        ..add(OnehundredtwoInitialEvent()),
      child: OnehundredtwoScreen(),
    );
  }
  ...
  Widget _buildProductCard(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(left: 2.h),
      child: BlocSelector<OnehundredtwoBloc, OnehundredtwoState,
          OnehundredtwoModel?>(
        selector: (state) => state.onehundredtwoModelObj,
        builder: (context, onehundredtwoModelObj) {
          return ListView.separated(
            ...
            itemCount: onehundredtwoModelObj?.productcard6ItemList.length ?? 0,
            itemBuilder: (context, index) {
              Productcard6ItemModel model =
                  onehundredtwoModelObj?.productcard6ItemList[index] ??
